...
def predict(x,beta):
    ...
    """
    return np.dot(beta.T, x)


""" Question 2 using f and localGradient """
def f(x,y,beta):
    ...
    """
    return np.square(y - np.dot(beta.T,x))
...
def localGradient(x,y,beta):
    ...
    """
    return -2.0 * (y - np.dot(beta.T,x)) * x
 

""" Question 3 using F and gradient """
def F(data,beta,lam = 0):
    ...
    """
    return float(data.map(lambda (x,y): f(x, y, beta)).sum() / data.count() + lam*np.dot(beta.T,beta))
...
def gradient(data,beta,lam = 0):
    ...
    """
    raw_result = data.map(lambda (x,y): localGradient(x, y, beta)).reduce(lambda elem1,elem2: [elem1[i]+elem2[i] for i in range(len(beta))])  
    return [x / data.count() for x in raw_result] + 2*lam*beta


""" Question 4 using F and gradient; test with MSE which is without regularization"""
def test(data,beta):
    ...
    """
    return float(data.map(lambda (x,y): f(x, y, beta)).sum() / data.count())
...
def train(data, beta_0, lam, max_iter, eps):
    ...
    P = len(beta_0)
    beta = beta_0
    gamma_func = TangAppro2D(data,lam)
    for i in range(max_iter):
        oldGrad = gradient(data, beta, lam)
        beta -= lineSearch(gamma_func.func, beta, oldGrad) * oldGrad
        currentGrad = gradient(data, beta, lam)
	currentGradNorm = np.dot(currentGrad.T, currentGrad)
	print 'Algorithm ran for ', i, ' iterations.'
        print 'Present Loss: ', np.round(F(data, beta, lam),6), ' Present Gradient Norm: ', np.round(currentGradNorm, 6)  
        print 'With current time: ', time.asctime( time.localtime(time.time()) )
	if currentGradNorm < eps:
	    return beta, currentGradNorm, i
    return beta, currentGradNorm, i

""" Tangent Estimation in training"""
def trainEstimate(data, beta_0, lam, max_iter, eps):
    N = data.count()
    P = len(beta_0)
    beta = beta_0
    loss_func = TangAppro2D(data, lam)
    gamma_func = TangAppro2D(data,lam)
    for i in range(max_iter):
        oldGrad = estimateGrad(loss_func.func, beta, 0.00001)
        beta -= lineSearch(gamma_func.func, beta, oldGrad) * oldGrad
        currentGrad = estimateGrad(loss_func.func, beta, 0.00001)
	currentGradNorm = np.dot(currentGrad.T, currentGrad)
	print 'Algorithm ran for ', i, ' iterations.'
        print 'Present Loss: ', np.round(F(data, beta, lam),6), ' Present Gradient Norm: ', np.round(currentGradNorm, 6)  
        print 'With current time: ', time.asctime( time.localtime(time.time()) )
...
class TangAppro1D:
    ''' Tangent Approximation:
        x: input vector with one dimension
  	y: target, which is a scaler
	beta: weights
	def __init__: with 2 arguments, x and y 
	def func: with 1 argument, beta, and run function f  '''
    def __init__(self, x, y):
        self.x = x
        self.y = y
    def func(self, beta):
        return f(self.x, self.y, beta)
    
class TangAppro2D:
    ''' Tangent Approximation:
        data: input dataframe, ([...],val)
  	lam: lambda for regularization
	beta: weights
	def __init__: with 2 arguments, data and lambda 
	def func: with 1 argument, beta, and run function F '''
    def __init__(self, data, lam=0):
        self.data = data
        self.lam = lam
    def func(self, beta):
        return F(self.data, beta, self.lam)
