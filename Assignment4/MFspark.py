...
def train(args, folds, latent, lam, mu):
    ...
    for k in folds:
        ...
        U = generateUserProfiles(train,int(latent),args.seed,sc,args.N).cache()
        V = generateItemProfiles(train,int(latent),args.seed,sc,args.N).cache()
        ...
        while i<args.maxiter and change > args.epsilon:
            ...
            oldObjective = obj
            obj = SE(joinedRDD) + normSqRDD(U,lam) + normSqRDD(V,mu)         
...
if __name__=="__main__":
    ...
    parser.add_argument('--plotCSV', default='plotCSV_1', help='File that write latent dimensions corresponding CV RMSE')
    ...
    for latent in args.latents: 
      for re in args.regul:
        cv_rmses = train(args, folds, latent, re, re)
        cur_rmse = np.mean(cv_rmses)
        if minrmse > cur_rmse:
          minrmse = cur_rmse
          bestlatent = latent
          bestre = re
        print "Latent %d, regularization %f, average error is: %f " % (latent, re, cur_rmse)
        
        storeplot = [latent, re, np.mean(cv_rmses)]
